...
public class HPTextUI : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI hpText; // Canvas UI용 TMP
    ...
    private void Start()
    {
        ...
        if (playerStats != null)
        {
            ...
            // 초기 텍스트 설정
            UpdateHPText(playerStats.CurrentHP, playerStats.MaxHP.Value);
        }
    }
    ...
    private void UpdateHPText(float currentHP, float maxHP)
    {
        ...
        hpText.text = string.Format(format, Mathf.CeilToInt(currentHP), Mathf.CeilToInt(maxHP));
    }
}