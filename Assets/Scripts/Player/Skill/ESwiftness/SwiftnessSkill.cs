...
public class SwiftnessSkill : MonoBehaviour, ISkill
{
    ...
    public float Cooldown => skillData != null ? skillData.cooldown : 8f;
    ...
    private IEnumerator SwiftnessRoutine(GameObject owner, PlayerStats stats)
    {
        ...
        stats.MoveSpeed.Multiply(skillData.speedMultiplier);

        float elapsed = 0;
        while (elapsed < skillData.buffDuration)
        {
        ...
        // 기본 효과 원복
        stats.MoveSpeed.Divide(skillData.speedMultiplier);
